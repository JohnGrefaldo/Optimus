...
function stepFn(results, parserHandle)
{
	...
	parser = parserHandle;
		stuff['members'] =  JSON.stringify({email_address:results.data[0].EMAIL,
									status:"subscribed",
									merge_fields:{
									ADDRESS:results.data[0].ADDRESS,
									GENDER:results.data[0].GENDER,
									AGE:results.data[0].AGE,
...
function bye(){
	$('#loading').modal('show');
	 ...
	 $.ajax({
	 	url: 'http://optimus.growthhacklife.com/test',
	 	method: 'POST',
	 	data: stuff,
